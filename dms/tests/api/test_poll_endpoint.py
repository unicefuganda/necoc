...
class TestPollEndpoint(MongoAPITestCase):
    ...
    def setUp(self):
        ...
        self.ludara_subcounty = Location(**(dict(name='LUDARA', parent=self.amuru.id, type='subcounty'))).save()
        ...
        self.poll_to_post3 = dict(name="Disaster3", question="How many disasters are in your area?", keyword="some_word3",
    ...
    def test_ddmc_should_get_a_list_of_polls_of_his_district(self):
        ...
        user_attr = dict(name='sam', phone='+256775019441', location=self.ludara_subcounty, email=None, user=user)
        UserProfile(**(user_attr)).save()
        poll = Poll(**self.poll_to_post).save()
